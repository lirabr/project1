...
.cover h1 { font-size: 52pt; line-height: 1.13; margin-top: 26px; }
.cover .subtitle { font-size: 19pt; max-width: 950px; }
.cover .content { align-items: stretch; flex-direction: column; justify-content: center; gap: 28px; }
.toc { display: grid; grid-template-columns: 1fr 1fr; gap: 17px 45px; font-size: 16pt; }
.toc div { border-bottom: 1px solid #d6e1ec; padding: 13px 0; }
  ...
  try {
    ...
    const sizes = await page.evaluate(async (diagrams) => {
      ...
      for (const sheet of document.querySelectorAll('.sheet')) {
        if (sheet.scrollHeight > sheet.clientHeight + 2) throw new Error('Page content overflow');
        const body = sheet.querySelector('.body');
        if (body && body.scrollHeight > body.clientHeight + 2) throw new Error('Body overlaps footer');
      }
      return stats;
    }, input.diagrams);
...
def wrap_diagram(source: str) -> str:
    def quoted(match):
        return '"' + '<br/>'.join(textwrap.wrap(match.group(1), width=29)) + '"'
...
def build_document() -> tuple[str, list[str], list[str]]:
    ...
    originals = re.findall(r"```mermaid\n(.*?)```", visual, re.DOTALL)
    if len(originals) != 5:
        raise ValueError("Expected five architecture diagrams; review the export against changed sources")
    for name in ("langgraph-integration.md", "requirements-and-contracts.md", "roadmap.md"):
        if not (ROOT / name).is_file():
            raise FileNotFoundError(name)
    authority_lines = originals[1].strip().splitlines()
    authority_first = '\n'.join(authority_lines[:5])
    authority_second = 'flowchart LR\n  H["Human approval bound to intent hash"]\n' + '\n'.join(authority_lines[5:])
    phase_lines = originals[2].strip().splitlines()
...
'''
    specs = [
        ("Domains and shared controls", "TARGET ARCHITECTURE / Six domains, one modular deployment", originals[0],
        ...
        ("State ownership and memory", "TARGET DATA RESPONSIBILITIES / Checkpoints are not portfolio truth", originals[3],
        ...
        ("Verification and acceptance", "TARGET TEST PYRAMID / Evidence before external execution", originals[4],
         "The starter automates portions of unit and synthetic integration testing. Broker-paper certification, operational observation and independent release review remain separate gates.", "architecture-visual.md / Test pyramid"),
        ("LangGraph workflow", "IMPLEMENTED PROTOTYPE / Internal paper only", graph,
         "A graph may pause for each allowed symbol. Approval expires after 300 seconds from the snapshot by default. Risk and HALT are rechecked; the local book still lacks production accounting and broker reconciliation.", "langgraph-integration.md / Shipped topology"),
...
def build_overview() -> tuple[str, list[str], list[str]]:
    ...
    * { box-sizing: border-box; }
    ...
    h2 { font-size: 13pt; margin: 16px 0 7px; }
    p { margin: 0 0 10px; }
    ...
    <div class="note">These layers describe the <b>target design</b>. The current prototype
    implements parts of them; full portfolio accounting, execution and operational safeguards
    still require development and certification.</div>
    """
    phases = [
    ...
    <div class="grid status">
      <div class="card"><b>Available in the starter</b><ul>
        <li>Local research, features and walk-forward screening.</li>
        <li>Rule-based reports, basic gates and a toy internal-paper book.</li>
        <li>Optional graph approvals, memory diagnostics and regression tests.</li>
      </ul></div>
      <div class="card"><b>Still required for operation</b><ul>
        <li>Accurate marked/session accounting and realistic trade exits.</li>
        <li>Approved inference artifacts and time-correct memory.</li>
        <li>Broker reconciliation, scoped approvals, durable audit and recovery.</li>
      </ul></div>
    </div>
    ...
    """
    titles = ["Trading Desk v3", "The architecture, simplified", "From idea to reliable operation"]
    subtitles = ["Project overview · idea, purpose and intended value", "Six domains, shared controls and a small starting stack", "The phased roadmap and an honest view of readiness"]
    sources = ["README.md · trading-agents-architecture.md", "trading-agents-architecture.md · architecture-visual.md", "roadmap.md · README.md · phase-5-operations.md"]
    pages = []
    for index, (title, subtitle, content, source) in enumerate(zip(titles, subtitles, (first, second, third), sources, strict=True), 1):
        pages.append(
